"""
Trustpilot web scraper date written 07/08/2019
"""
...
def getLocation(link):
	location_html = requests.get('https://www.trustpilot.com'+link).text
	location_soup = BeautifulSoup(location_html,'html5lib')

	for div in location_soup('div','user-summary-location'):
		location = div.text
...
for i in range(1,251):
	...
	df_reviews.to_csv('asos_reviews_v2.csv',mode='a',header=False)
...
for link in extra_reviews_users:
	...
	for x in supp_scores:
		supp_userids.append(link)
...
for idx,score in enumerate(supp_scores):
	supp_dicts.append({'userid':supp_userids[idx],
		'rating':score,
...
df_supp_reviews.to_csv('supplementary_reviews_v2.csv',mode='a',header=False)